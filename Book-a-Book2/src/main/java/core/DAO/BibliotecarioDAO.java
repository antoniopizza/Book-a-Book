...
public class BibliotecarioDAO extends AbstractDAO<Bibliotecario>{
    private final String doDeleteBibliotecarioQuery = "DELETE FROM Bibliotecario WHERE id = ?";
   ...
   public int doDelete(String isil) {
    ...
    public int doDeleteBibliotecario(Bibliotecario b) {
        try{
            Connection con = DriverManagerConnectionPool.getConnection();            
            PreparedStatement prst = con.prepareStatement(doDeleteBibliotecarioQuery);
            prst.setInt(1, b.getId());
